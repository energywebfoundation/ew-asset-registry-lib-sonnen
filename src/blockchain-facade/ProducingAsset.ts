...
export const createAsset =
    async (assetPropertiesOnChain: OnChainProperties,
        ...
        const tx = await configuration.blockchainProperties.producingAssetLogicInstance.createSonnenAsset(
            ...
            assetPropertiesOnChain.marketLookupAddress,
            {
...
export class Entity extends Asset.Entity implements OnChainProperties {
    ...
    async sync(): Promise<Entity> {
        if (this.id != null) {
            ...
            const asset = await this.configuration.blockchainProperties.producingAssetLogicInstance.getSonnenAssetById(this.id);
    ...
    async saveSmartMeterRead(meterReading: number,
                             filehash: string,
                             timeFrameFrom: number,
                             timeFrameTo: number,
                             averagePower: number,
        ...
        if (this.configuration.blockchainProperties.activeUser.privateKey) {
            return this.configuration.blockchainProperties.producingAssetLogicInstance.saveSonnenSmartMeterRead(
                this.id,
                meterReading,
                filehash,
                timeFrameFrom,
